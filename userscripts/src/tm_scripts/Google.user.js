...
(function () {
  ...
  function toggleAIMode() {
    function toggleTutorialModeSetting(localStorageKey) {
      ...
      applyAIMode();
      console.log(
        `AI mode is now ${newValue ? "ON" : "OFF"}, localStorage toggled`,
        location.host,
      );
    ...
    CSJ_UTILS.controlPlusShiftPlusSomeKeyOnSubmit("u", () =>
      toggleTutorialModeSetting(AI_MODE_TOGGLE_KEY),
    );
  }

  async function applyAIMode() {
    const aiModeFlag = JSON.parse(
      localStorage.getItem(AI_MODE_TOGGLE_KEY) || "false",
    );
    await new Promise((r) => setTimeout(r, 200));

    const aiModeButton = document.querySelector("[data-mstk-u]");
    const allButton = Array.from(
      document.querySelectorAll(`[href*="/search?q="]`),
    ).find((item) => item.textContent.toLowerCase().includes("all"));

    const isInAIMode = !document.querySelectorAll("[aria-disabled][data-hveid]")
      .length;
