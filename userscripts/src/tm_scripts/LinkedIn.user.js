...
(async function () {
        ...
        document.addEventListener("keydown", async function (e) {
            try {
                if (e.key === "Enter" && e.shiftKey) {
                    ...
                    const isEasyApplyModalShowing = !!document
                        .querySelector('[data-test-modal-container]');

                    if (!isEasyApplyModalShowing) {
                        // when done, move to next
                        const recentlyAppliedText = document.querySelector(".job-details-jobs-unified-top-card__container--two-pane .artdeco-inline-feedback--success")?.textContent?.toLowerCase?.() || "";
                        if (recentlyAppliedText.includes("applied")) {
                            const nextItem = document.querySelector("li:has(.jobs-search-results-list__list-item--active) + li")?.querySelector?.(".job-card-container__link");
                            if (!nextItem) console.log("next item now found");
                            nextItem?.click?.();

                        } else {
                            // apply button easy
                            document.querySelector(".jobs-apply-button--top-card [data-live-test-job-apply-button]")?.click?.();

                            // hit yes if that didnt work
                            document.querySelector('[aria-label="Did you apply?, Yes"]')?.click?.();
                        }
                        return;
                    }

                    const button = document
                        .querySelector('[data-test-modal-container] .artdeco-button--primary');
                    ...
                    if (button.textContent.toLowerCase().includes("submit")) {
                        ...
                        document
                            .querySelector("#follow-company-checkbox")?.click(); // unfollow
                        console.log("Checkbox clicked, hopefully unticked now");
                        button.click();
                    } else if (button.textContent.toLowerCase().includes("show jobs")) {
                        // dismiss
                        document.querySelector("[data-test-modal-container] [aria-label='Dismiss']")?.click?.();
                    }
                    else {
                ...
                if (e.key === "|" && e.shiftKey) {
                    ...
                    const button = document
                        .querySelector('[data-test-modal-container] .artdeco-button--secondary');
    ...
    window?.isSiteLoaded?.(function hideApplied() {
        window.sleep(1500);
        const jobs = Array.from(document.querySelectorAll("[data-occludable-job-id]")).map(job => ({ item: job, title: ".job-card-list__title--link", company: document.querySelector(".artdeco-entity-lockup__subtitle"), applied: job.querySelector(".job-card-container__footer-item.job-card-container__footer-job-state.t-bold").textContent.toLowerCase().includes("applied") }));
        jobs.forEach(box => {
            if (box.applied) {
    ...
    window?.isSiteLoaded?.(function hideApplied() {
        window.sleep(1500);
        const button = document.querySelector("[aria-label='Click to see more description']");
        button?.click?.();
    })

    function markAppliedViewed() {
        document.querySelectorAll(".job-card-container--clickable").forEach(item => {
    ...
    async function hideAll() {
        for (const item of Array.from(document.querySelectorAll(`button[aria-label*="Dismiss "]`))) {
            item.click();
