...
(function() {
    ...
    function detectEventCard() {
        document.addEventListener("click", async (event) => {
            if (!event.target.parentElement.getAttribute("data-opens-details")) return;
            ...
            if (!!document.querySelector("[data-actions-expanded]")) {
                function buttonOnClick () {
                    try {
                        ...
                        const timeText = document.querySelector("[data-text]:has([role='heading']) + div span:last-child").textContent;
