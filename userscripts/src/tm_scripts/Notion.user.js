...
(function () {
  ...
  function toggleCommentOrder() {
    const commentContainer = document.querySelector(
      ".notion-update-sidebar-tab-comments-comments-scroller > div[role='menu']",
    );
    if (!commentContainer) {
    ...
    commentContainer.style.flexDirection =
      commentContainer.style.flexDirection == "column-reverse"
        ? "column"
        : "column-reverse";
  ...
  async function editShortcut() {
    ...
    try {
      ...
      function getOverlappingElements(
        containerSelector,
        targetSelector = `[aria-label="Comment actions"] [aria-label="More actions"]`,
      ) {
      ...
      const kebabButton = uiScope.querySelector(
        `[aria-label="Comment actions"] [aria-label="More actions"]`,
      );
      if (!kebabButton) {
      ...
      document
        .querySelector(`.notion-comment-overflow-menu .pencilLine`)
        .closest("div")
        .click();
    } catch (e) {
      console.log(e);
