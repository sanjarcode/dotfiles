...
(function () {
    ...
    function attachToSanjarWindow(
        ...
        try {
            ...
            if (callOnce) {
                if (f == isSiteLoaded) isSiteLoaded(); // run itself.
    ...
    function handlePhoneLinks() {
        ...
        async function interceptor(event) {
            ...
            try {
                ...
                window.showNotification("Dial failed ❌. Copied to clipboard!" + phoneNumber + "...");
                try {
                    ...
                    window.open(`whatsapp://send/?phone=${encodeURIComponent(phoneNumber)}&text&type=phone_number&app_absent=0&closeFast=true`);
                } catch (whatsappError) {
                    window.showNotification("WhatsApp failed ❌. Copied to clipboard!" + phoneNumber + "...");
    ...
    async function setupClickNQueue(delayMs = 500) {
        await importHelper(`import pLimit from "https://esm.sh/p-limit"; window.pLimit = pLimit;`);
     ...
     * @param {*} freq
     */
    async function isSiteLoaded(f = () => { }, extraCriteria = () => true, timeout = 1000, freq = 10) {
        const defaultCriteria = [() => document.readyState == 'complete']; //
        let timeSpent = 0;
        const interval = setInterval(() => {
            if (timeSpent >= timeout) {
                const message = timeSpent >= timeout ? "isSiteLoaded check timed out" : "Site loaded detected";
    ...
    function splitLines(target = document.body) {
        ...
        const ancestor = typeof target === 'string'
        ...
        for (const node of textNodes) {
            ...
            sentences.forEach((s, i) => {
                ...
                if (i !== sentences.length - 1) frag.appendChild(document.createElement('br'));
            });
    ...
    function toggleStudyPauseMode() {
        ...
        window.controlPlusShiftPlusSomeKeyOnSubmit("u", () => toggleTutorialModeSetting(TUTORIAL_MODE_TOGGLE_KEY))
    }

    function extractForm(formEl = document.querySelector("form"), useStore = true) {
        const out = {};
        ...
        if (useStore) localStorage.setItem("tm_extracted_form", JSON.stringify(out));
        return out;
    }

    function fillForm(formEl = document.querySelector("form"), values = null, useStore = true) {
        ...
        for (const [key, val] of Object.entries(values)) {
            ...
            if (!el) {
                const label = [...formEl.querySelectorAll("label")]
                    .find(l => l.textContent.trim().toLowerCase() === key.trim().toLowerCase());
        ...
        class EmojiAnnotator {
            ...
            wholePage(root = document.body) {
                ...
                if (this.logger_enabled) {
                    console.log(`[EmojiAnnotator] wholePage: annotated ${count} emoji nodes.`);
                }
    ...
    async function uploadFile(selector = 'input#resume', file_url = 'http://localhost:8085/sanjar-afaq-resume.pdf') {
    ...
    function ttl_add(item) {
        localStorage.setItem("ttl_selectors", JSON.stringify([...JSON.parse(localStorage.getItem("ttl_selectors") || "[]"), item]));
    }
    function ttl_show() {
