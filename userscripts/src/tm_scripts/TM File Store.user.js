...
async function upload_tm_file() {
  return new Promise((resolve) => {
    ...
    input.onchange = async () => {
      ...
      const results = await Promise.all(files.map(file => {
        return new Promise((res, rej) => {
          ...
          reader.onload = () => {
            ...
            GM_setValue(key, b64);
            console.log(`[TM File Store] Stored: "${file.name}" (${(file.size / 1024).toFixed(1)} KB)`);
            res(file.name);
          };
      ...
      console.log(`[TM File Store] Done. ${results.length} file(s) stored:`, results);
      _renderFileList();
      resolve(results);
    };
...
function _renderFileList() {
  ...
  if (!names.length) {
    listEl.innerHTML = '<div style="color:#999;font-size:11px;">No files stored</div>';
    return;
  }
...
function _showPanel() {
  ...
  if (panel) { panel.style.display = 'block'; _renderFileList(); }
}
...
function _injectUI() {
  ...
  document.getElementById('_tmfs_upload').addEventListener('click', upload_tm_file);
