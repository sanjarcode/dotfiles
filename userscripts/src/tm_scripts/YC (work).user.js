  ...
  function addAgoDate() {
    ...
    if (window.location.href.includes("/companies/")) {
      const jobs = JSON.parse(
        document
          .querySelector(`[id*='Full'][data-page]`)
          .getAttribute("data-page"),
      ).props.rawCompany.jobs;

      document.querySelectorAll(".job-name").forEach((item, idx) => {
        const created_at = jobs[idx].jobs_search.created_at;
        item.innerHTML =
          item.innerHTML.split("{").at(0) +
          ` <span style="color: red;" title="${new Date(created_at).toDateString().split(" ").slice(1).join(" ")}">{${formatRelativeDate(new Date(created_at))}}</span>` +
          ` <span style="color: green; font-size: 12px;">{${formatRelativeDate(new Date(jobs[idx].pretty_updated_at))}}</span>`;
      });
    } else {
