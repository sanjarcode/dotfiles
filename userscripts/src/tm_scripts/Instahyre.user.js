...
(function () {
    ...
    async function addAgoDate(hideOlder = true) {
        ...
        if (window.location.href.includes("/job-")) {
            const datePosted = JSON.parse(Array.from(document.querySelectorAll(`script[type="application/ld+json"]`)).find(item => item.textContent.includes("title")).textContent).datePosted;
        ...
        if (onResultsPage) {
            let jobs = [];
            try {
                jobs = JSON.parse(Array.from(document.querySelectorAll(`[type="application/ld+json"]`))?.find?.(item => item.textContent.includes("/job-"))?.textContent)?.itemListElement;
                document.querySelectorAll(".employer-row").forEach(async (row, idx) => {
                    ...
                    await q(() => {
                        title.innerHTML = ` <span style="color: red;" class="added-date" title="${new Date(gottenDate).toDateString().split(" ").slice(1).join(" ")}">{${formatRelativeDate(new Date(gottenDate))}}</span>}` + (title.innerHTML.split("</span>}").at(1) || title.innerHTML);
                    });
                    console.log(`Trying job idx / ${jobs.length} ${gottenDate}`, jobs[idx]?.url);
                    if (hideOlder && isOld) {
                        //
                        // document.querySelector(`.employer-row:has([title='${title.getAttribute("title")}'])`)?.remove?.();
                        // 
                        const notInterested = row.querySelector(`[id="not-interested-btn"]`);
    ...
    if (document.querySelectorAll(`button#show-results`).length)
        Array.from(document.querySelectorAll(`button#show-results`))[0].addEventListener("click", addAgoDate);
