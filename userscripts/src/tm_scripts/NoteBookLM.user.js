...
(function () {
    ...
    async function expandAll(node, selector = ".expand-symbol", stoppingThreshold = 1000, gap = 100) {
    ...
    function setupSaveNoteShortcut() {
        window.controlPlusShiftPlusSomeKeyOnSubmit("u", () => {
            const lastNoteButton = Array.from(document.querySelectorAll("button[title='Pin this message to a Note']")).at(-1);
    ...
    function getFlashCards() {
        ...
        if (!flashCardNode) {
            ...
            window.copy((JSON.parse(flashCardNode.getAttribute("data-app-data"))?.flashcards ?? []).map(item => `${item.f}|${item.b}`).join("\n"));
        }
    }

    function handleSourceOrder() {
        function setSourceOrder() {
            Array.from(Array.from(document.querySelector(".scroll-area-desktop").children)).sort((a, b) => a.textContent.localeCompare(b.textContent, undefined, {numeric: true})).forEach((item, index) => {item.style.order = index; console.log(item.textContent) });
        }
